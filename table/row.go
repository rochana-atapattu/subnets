...
func (m Model) renderRowData(row Row, rowIndex int, rowStyle lipgloss.Style, last bool) string {
	...
	for columnIndex, column := range m.columns {
		...
		highlighted := rowIndex == m.rowCursorIndex && columnIndex == m.columnCursorIndex

		if m.focused && highlighted {
			cellStyle = rowStyle.Inherit(m.highlightStyle)
		}

		cellStr := m.renderRowColumnData(row, column, cellStyle, borderStyle)
		...
		if m.maxTotalWidth != 0 {
			...
			if totalRenderedWidth+renderedWidth > targetWidth {
				...
				overflowStr := m.renderRowColumnData(row, overflowColumn, cellStyle, overflowStyle)
