...
type UserEventHighlightedIndexChanged struct {
	...
	// PreviousColumnIndex is the column that was selected before the change.
	PreviousColumnIndex int

	// SelectedColumnIndex is the column index that is now SelectedRowIndex
...
type UserEventColumnSortToggled struct {
	ColumnIndex int
	IsSelected bool
}
